...
// Step 1: Sort oldest to newest to compute cumulative
$sorted_oldest = array_reverse($all_rows);
foreach ($sorted_oldest as &$row) {
    $cumulative_savings += $row['savings'] ?? 0;
    $row['cumulative_savings'] = $cumulative_savings;
}
unset($row);

// Step 2: Reverse again to display from latest to oldest
$display_rows = array_reverse($sorted_oldest);
...
<?php endforeach; ?>
        ...
        <tfoot>
            <tr class="totals">
                ...
                <td><?= number_format($totals['loan_balance'], 2) ?></td>
